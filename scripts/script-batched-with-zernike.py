...
def build_model_zernike(n, variance, bw=False, K=64, width=128, depth=5):
    """
    - n: total input dims (x,y,lat,r)
    - K: number of Zernike basis functions to compute per-point
    - width, depth: MLP size
    """
    # 1) Precompute a fixed table of (n_r, n_m) indices for the first K Zernike modes.
    modes = [(p, q) for p in range(0, 16) for q in range(-p, p+1, 2)]
    modes = modes[:K]
    
    inp = layers.Input(shape=(n,), name="inp")
    # 2) Extract polar coords
    def to_zernike(x):
        # x shape (batch, n); we assume x[:,0:2] = (x,y)
        xy = x[:, :2]
        r = tf.sqrt(tf.reduce_sum(xy**2, axis=1, keepdims=True))
        θ = tf.atan2(xy[:,1:2], xy[:,0:1])
        feats = []
        for p, q in modes:
            # radial polynomial R_p^|q|(r)
            # you can use a recursive definition or implement via tf.special
            Rpq = tf.math.pow(r, tf.cast(p, tf.float32))  # crude: r^p  (for illustration)
            if q >= 0:
                feats.append(Rpq * tf.cos(tf.cast(q, tf.float32) * θ))
            else:
                feats.append(Rpq * tf.sin(tf.cast(-q, tf.float32) * θ))
        return tf.concat(feats, axis=1)  # (batch, K)
    zf = layers.Lambda(to_zernike, name="zernike_feats")(inp)

    # 3) Concatenate extras (lat, r, etc)
    extras = layers.Lambda(lambda x: x[:,2:], name="extras")(inp)
    x = layers.Concatenate()([zf, extras])  # (batch, K + (n-2))

    # 4) Small tanh‐MLP
    init = initializers.VarianceScaling(scale=variance)
    for i in range(depth):
        x = layers.Dense(width, kernel_initializer=init, activation='tanh',
                         name=f"mlp_{i}")(x)

    out_ch = 1 if bw else 3
    x = layers.Dense(out_ch, activation='tanh', kernel_initializer=init,
                     name="final")(x)

    model = models.Model(inp, x, name="zernike_mlp")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--x', type=int, nargs='?', default=400)
    parser.add_argument('--y', type=int, nargs='?', default=400)
    ...
    for i in tqdm(range(args.n)):
        ...
        lat = np.random.normal(0, 1, 1)
        model = build_model_zernike(len(create_grid((args.x, args.y), lat)), variance)
