...
def build_model_conv(z_dim=64, bw=False):
    """
    A small convolutional generator:
      - Input: a latent vector z ∈ ℝ^{z_dim}
      - Dense → reshape to e.g. 8×8×(z_dim//2)
      - Upsample 4× → 16×16, 32×32, 64×64, 128×128, 256×256 or even more
      - Output final image (x_dim × y_dim × 3) via tanh
    """
    init = initializers.VarianceScaling(scale=50.0)
    z = layers.Input(shape=(z_dim,))  # latent code

    # Project & reshape into a small spatial map
    x = layers.Dense(8 * 8 * (z_dim // 2), kernel_initializer=init)(z)
    x = layers.Reshape((8, 8, z_dim // 2))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)

    # Upsample blocks (Conv2DTranspose)
    filters = z_dim // 4
    for size in [16, 32, 64, 128]:  # you can adjust the sequence to match args.x/args.y
    ...
    # Final conv → 1 or 3 channels
    out_channels = 1 if bw else 3
    x = layers.Conv2DTranspose(out_channels, kernel_size=3, strides=1, padding='same',
                               activation='tanh',
                               kernel_initializer=initializers.RandomNormal(0, 0.02))(x)

    # If you need exactly (args.x, args.y) = (1600,1600), you may have to add more upsampling
    # or use a Resize layer after the final tanh to scale to exactly (1600,1600).

    model = models.Model(inputs=z, outputs=x)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(args.n)):
        ...
        lat = np.random.normal(0, 1, 1)
        model = build_model_conv(len(create_grid((args.x, args.y), lat)), variance)
        ...
        for j in tqdm(range(args.series), leave=False):
            ...
            image = create_image_batched(model, params, (args.x, args.y), batch_size=4096)
