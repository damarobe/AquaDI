...
def build_model(n, variance, bw=False, width=32, depth=8, activation=tf.tanh):
    """
    Builds a CPPN whose hidden layers form a DenseNet‐style block:
      - each new Dense layer takes as input the concatenation of
        all previous outputs (including the original input).
    Final output layer then maps that concatenated feature vector to 1 or 3 channels.

    Args:
      n         : input dimension (e.g. number of coordinate features per pixel)
      variance  : scale for VarianceScaling initializer
      bw        : if True, final layer has 1 output (grayscale). Otherwise 3 (HSV → RGB).
      width     : number of units in each hidden Dense layer
      depth     : number of hidden layers
      activation: activation for all hidden layers (default tf.tanh)

    Returns:
      A compiled Keras Model that takes shape=(n,) → output shape=(1,) or (3,)
    """
    init = initializers.VarianceScaling(scale=variance)
    inp = layers.Input(shape=(n,))   # original “h₀”

    # We'll keep a running “x” that starts at inp,
    # and after each hidden layer we concatenate the new output.
    x = inp

    for _ in range(depth):
        # 1) apply Dense to the current x → get y
        y = layers.Dense(width,
                         kernel_initializer=init,
                         activation=activation)(x)
        # 2) concatenate the old x and y → new x has (old_dim + width) dims
        x = layers.concatenate([x, y])

    # After depth hidden layers, x has dimension:  n + depth*width
    # Now map that directly to 1 or 3 outputs with tanh
    out_channels = 1 if bw else 3
    out = layers.Dense(out_channels, activation='tanh')(x)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(args.n)):
        ...
        lat = np.random.normal(0, 1, 1)
        model = build_model(len(create_grid((args.x, args.y), lat)), variance, bw=args.bw)
