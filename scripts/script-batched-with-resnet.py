...
def build_model_resnet(n, variance, bw=False, width=128, depth=6, activation=tf.tanh):
    """
    A small ResNet‐style MLP: 
      1) project input (n) → (width) with Dense  
      2) repeat `depth` ResBlocks:  
         ResBlock(x): y = activation(Dense(width)(x)); return x + y  
      3) final Dense → 1 or 3 channels + tanh  
    """
    init = initializers.VarianceScaling(scale=variance)
    inp = layers.Input(shape=(n,))

    # 1) initial projection to match `width`
    x = layers.Dense(width, kernel_initializer=init, activation=activation)(inp)

    # 2) residual blocks
    for _ in range(depth):
        y = layers.Dense(width, kernel_initializer=init, activation=None)(x)
        y = activation(y)
        x = layers.Add()([x, y])  # x ← x + y

    # 3) final mapping back to 1 or 3 dims
    out_channels = 1 if bw else 3
    x = layers.Dense(out_channels, activation='tanh',
                     kernel_initializer=initializers.VarianceScaling(scale=variance))(x)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(args.n)):
        ...
        lat = np.random.normal(0, 1, 1)
        model = build_model_resnet(len(create_grid((args.x, args.y), lat)), variance, bw=args.bw)
