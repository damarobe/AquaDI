...
def build_model_siren(n, variance, bw=False, width=256, depth=5, ω0=30.0):
    """
    A SIREN‐style MLP: each Dense layer → sin(ω0·(W x + b)) for the first layer,
    then sin(W x + b) for subsequent layers. Finally outputs 1 or 3 channels via tanh.
    """
    # Custom initializer for sine layers:
    class SineInitializer(initializers.RandomUniform):
        def __init__(self, is_first=False, ω0=30.0):
            self.is_first = is_first
            self.ω0 = ω0
            # For the first layer, range = [-1/ n, 1/ n], then multiplied by ω0
            limit = (1.0 / n) if is_first else (math.sqrt(6.0 / n) / self.ω0)
            super().__init__(minval=-limit, maxval=limit)

        def __call__(self, shape, dtype=None):
            vals = super().__call__(shape, dtype)
            return vals * (self.ω0 if self.is_first else 1.0)

    inp = layers.Input(shape=(n,))
    x = inp

    # — First SIREN layer (with ω0 scaling)
    x = layers.Dense(width,
                     kernel_initializer=SineInitializer(is_first=True, ω0=ω0),
                     bias_initializer='zeros')(x)
    x = tf.sin(x)

    # — Subsequent SIREN layers
    for _ in range(depth - 1):
        x = layers.Dense(width,
                         kernel_initializer=SineInitializer(is_first=False, ω0=ω0),
                         bias_initializer='zeros')(x)
        x = tf.sin(x)

    out_channels = 1 if bw else 3
    x = layers.Dense(out_channels, activation='tanh',
                     kernel_initializer=initializers.RandomUniform(-math.sqrt(6/n), math.sqrt(6/n)),
                     bias_initializer='zeros')(x)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(args.n)):
        ...
        lat = np.random.normal(0, 1, 1)
        model = build_model_siren(len(create_grid((args.x, args.y), lat)), variance, bw=args.bw)
