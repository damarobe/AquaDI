...
def build_model_gp(n, variance, bw=False, num_rff=512):
    """
    GP‐inspired Random Fourier Features network:
      - n: per‐pixel input dim (e.g. 4)
      - variance: used as a scale for the final linear layer initializer
      - bw: True → 1‐channel, False → 3‐channel (HSV→RGB)
      - num_rff: number of random Fourier features (d). Typically a few hundred.
    
    Workflow:
      1) Draw random matrix W ∈ ℝ^(n × num_rff), b ∈ ℝ^(num_rff), both non‐trainable.
      2) φ(p) = sqrt(2/num_rff) * cos(Wᵀ p + b).  → shape = (batch_of_pixels, num_rff)
      3) Final Dense(num_rff → out_channels) + tanh.
    """

    # 1) Create fixed random projections W, b
    W_np = np.random.normal(0.0, 1.0, size=(n, num_rff)).astype(np.float32)  # (n, d)
    b_np = np.random.uniform(0.0, 2.0 * np.pi, size=(num_rff,)).astype(np.float32)  # (d,)

    W = tf.constant(W_np)  # non‐trainable
    b = tf.constant(b_np)  # non‐trainable

    # 2) Build Keras model
    inp = layers.Input(shape=(n,), name="gp_input")

    def rff_layer(x):
        # x: (batch_of_pixels, n)
        # Compute x @ W → shape = (batch, num_rff)
        proj = tf.linalg.matmul(x, W)  # (b, d)
        proj = proj + b                # broadcast b: (d,) → (b, d)
        return tf.math.sqrt(2.0 / num_rff) * tf.cos(proj)  # (b, d)

    phi = layers.Lambda(rff_layer, name="rff_features")(inp)  # (batch_of_pixels, num_rff)

    # 3) Final linear map to out_channels + tanh
    ...
    model = models.Model(inputs=inp, outputs=out, name="gp_rff_network")
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(args.n)):
        ...
        lat = np.random.normal(0, 1, 1)
        model = build_model_gp(len(create_grid((args.x, args.y), lat)), variance, bw=args.bw)
